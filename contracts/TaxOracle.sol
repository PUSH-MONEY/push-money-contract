...
contract TaxOracle is Ownable {
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(push), "token needs to be push");
        uint256 pushBalance = push.balanceOf(pair);
        uint256 btcbBalance = btcb.balanceOf(pair);
        return uint144(pushBalance.mul(_amountIn).div(btcbBalance));
    }

    function getPushBalance() external view returns (uint256) {
    	return push.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 pushBalance = push.balanceOf(pair);
        uint256 btcbBalance = btcb.balanceOf(pair);
        return pushBalance.mul(1e18).div(btcbBalance);
    }

    function setPush(address _push) external onlyOwner {
        require(_push != address(0), "push address cannot be 0");
        push = IERC20(_push);
    }
